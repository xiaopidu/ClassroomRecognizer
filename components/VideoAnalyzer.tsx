...
const VideoAnalyzer: React.FC<VideoAnalyzerProps> = ({ students, params }) => {
  ...
  useEffect(() => {
    if (!isLoadingModels) {
      // Note: we now pass similarityThreshold, though updateFaceMatcher mainly updates the data store
      FaceRecognitionService.getInstance().updateFaceMatcher(students, params.similarityThreshold);
    }
  }, [students, params.similarityThreshold, isLoadingModels]);
  ...
  const updateAccumulation = (detections: any[]) => {
      ...
      detections.forEach(det => {
          ...
          let name = "unknown";
          if (det.bestMatch) {
              const match = det.bestMatch as { label: string; score: number };
          ...
          if ((det as any).isManual && det.bestMatch) {
             const match = det.bestMatch as { label: string };
             name = match.label; // Trust manual
          }
  ...
  const finishAccumulation = () => {
      ...
      if (canvasRef.current) {
         ...
         if (ctx) {
             ...
             FaceRecognitionService.getInstance().drawStableResults(ctx, validResults);
         }
      }
  };
  ...
  const handleSnapshot = async () => {
    ...
    try {
      await FaceRecognitionService.getInstance().analyzeFrame(
        videoRef.current,
        canvasRef.current,
        params,
        manualDetections 
